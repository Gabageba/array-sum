...
function sostavChisla(massivChisel, chislo) {
  ...
  function counter(arr, index, skip, currentCombination) {
    debugger
    if (index >= arr.length) {
      let sum = 0
      currentCombination.map((num) => {
        sum += num
      })
      if (sum === chislo) {
        result = [...result, currentCombination]
      }
    } else {
      counter(arr, index + 1, true, currentCombination)
      if (!(index > 0 && skip && arr[index] === arr[index - 1])) {
        currentCombination = [...currentCombination, arr[index]]
        counter(arr, index + 1, false, currentCombination)
      }
    }
  }

  counter(massivChisel, 0, true, [])
  return result
}
